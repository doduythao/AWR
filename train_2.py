...
class Trainer(object):
    def __init__(self, config):
        ...
        self.work_dir = osp.join(
            ...
            self.config.dataset,
            "checkpoint_" + self.config.exp_id,
        )
        ...
        self.log_file = osp.join(
            self.work_dir, "%s_%s.log" % (self.config.net, self.config.log_id)
        )
        ...
        if "resnet" in self.config.net:
            ...
            self.net = get_deconv_net(
                net_layer, self.config.jt_num, self.config.downsample
            )
        ...
        if self.config.scheduler == "auto":
            self.scheduler = ReduceLROnPlateau(
                self.optimizer, "min", patience=2, min_lr=1e-8
            )
            ...
            self.scheduler = StepLR(
                ...
                gamma=0.1,
                last_epoch=self.best_records["epoch"] - 1,
            )
    ...
    def train(self):
        ...
        # step4: meters
        eval_tool = EvalUtil(
            self.trainData.img_size,
            self.trainData.paras,
            self.trainData.flip,
            self.trainData.jt_num,
        )
        ...
        for epoch in range(self.best_records["epoch"] + 1, self.config.max_epoch + 1):
            self.net.train()
            for ii, (img, jt_xyz_gt, jt_uvd_gt, center_xyz, M, cube) in tqdm(
                ...
                offset_gt = self.FM.joint2offset(
                    jt_uvd_gt, input, self.config.kernel_size, self.ft_sz
                )
                ...
                if "hourglass" in self.config.net:
                    for stage_idx in range(self.stacks):
                        ...
                        jt_uvd_pred = self.FM.offset2joint_softmax(
                            offset_pred, input, self.config.kernel_size
                        )
                ...
                if (ii + 1) % self.config.print_freq == 0:
                    print_msg(
                        "[epoch: {:02d}][train loss: {:.5f}][offset_loss: {:.5f}][coord_loss: {:.5f}]".format(
                            epoch,
                            loss_meter.value()[0],
                            loss_offset.item(),
                            loss_coord.item(),
            ...
            print_msg(
                "[epoch {:02d}], [train loss {:.5f}], [train mpe {:.5f}], [lr {:.1e}]".format(
                    epoch,
                    loss_meter.value()[0],
                    train_mpe,
    ...
    def test(self, epoch=0):
        ...
        self.net.eval()
        eval_tool = EvalUtil(
            self.testData.img_size,
            self.testData.paras,
            self.testData.flip,
            self.testData.jt_num,
        )
        ...
        for ii, (img, jt_xyz_gt, jt_uvd_gt, center_xyz, M, cube) in tqdm(
            ...
            if (ii + 1) % self.config.vis_freq == 0:
                img_path = osp.join(
                    self.result_dir, "test_epoch_{}_iter_{}.png".format(epoch, ii + 1)
                )
                jt_uvd_pred_vis = (jt_uvd_pred[0] + 1) * self.config.img_size / 2.0
                jt_uvd_gt_vis = (jt_uvd_gt[0] + 1) * self.config.img_size / 2.0
                self.vis_tool.plot(
        ...
        eval_tool.plot_pck(
            osp.join(self.work_dir, "test_pck_epoch_{}.png".format(epoch)), pck, thresh
        )
        if epoch == 0:
            ...
            if not txt_file == None:
                np.savetxt(
                    txt_file,
                    jt_uvd.reshape([jt_uvd.shape[0], self.config.jt_num * 3]),
                    fmt="%.3f",
                )

        print_msg(
            "[epoch {:2d}], [test mpe {:.3f}], [lr {:.1e}]".format(
                epoch, mpe, self.optimizer.param_groups[0]["lr"]
            ),
            file=self.log,
        )
